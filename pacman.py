...
# Modified this so that the code would use the new ghost classes I created
...
def readCommand(argv):
    ...
    parser.add_option('-v', '--trainghosts', dest='trainghosts',
                      help=default(
                          'the ghost agent TYPE in the ghostAgents module to use'),
                      metavar='TYPE', default='TrainDirectionalGhost')
    parser.add_option('-w', '--testghosts', dest='testghosts',
                      help=default(
                          'the ghost agent TYPE in the ghostAgents module to use'),
                      metavar='TYPE', default='TestDirectionalGhost')                      
    ...
    trainghostType = loadAgent(options.trainghosts, noKeyboard)
    args['trainghosts'] = [trainghostType(i + 1) for i in range(options.numGhosts)]

    testghostType = loadAgent(options.testghosts, noKeyboard)
    args['testghosts'] = [testghostType(i + 1) for i in range(options.numGhosts)]
...
# Modified this function to optimize run time and computational efficiency 

def runGames(layout, pacman, trainghosts, testghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    import __main__
    ...
    for i in range(numGames):
        ...
        if beQuiet:
            ...
            rules.quiet = True
            # rules.quiet = False
            game = rules.newGame(layout, pacman, trainghosts,
